import XCTest
@testable import PackingWeightly

@MainActor
final class PackingWeightlyTests: XCTestCase {
    var store: Store!
    ...
    func testAddItemIncreasesCount() {
        ...
        let added = store.add(Bag(title: "Test", value2: 0))
        XCTAssertTrue(added)
    ...
    func testCannotAddBeyondFreeLimitWhenNotPro() {
        ...
        for i in 0..<Store.freeLimit {
            _ = store.add(Bag(title: "Item \(i)", value2: 0))
        }
        let result = store.add(Bag(title: "Overflow", value2: 0))
    ...
    func testDeleteRemovesItem() {
        let item = Bag(title: "ToDelete", value2: 0)
        _ = store.add(item)
    ...
    func testUpdateModifiesItem() {
        var item = Bag(title: "Original", value2: 0)
        _ = store.add(item)
        item.title = "Updated"
        store.update(item)
    ...
    func testDeleteAtOffsets() {
        _ = store.add(Bag(title: "A", value2: 0))
        _ = store.add(Bag(title: "B", value2: 0))
    ...
    func testPersistenceRoundTrip() {
        _ = store.add(Bag(title: "Persisted", value2: 0))
        store.save()
        let reloaded = Store()
