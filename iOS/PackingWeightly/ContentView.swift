        ...
        NavigationStack {
            ZStack {
                ...
                if store.items.isEmpty {
                    ContentUnavailableView("No Bags Yet", systemImage: "tray", description: Text("Tap + to log your first one."))
                } else {
                    List {
                        ForEach(store.items) { item in
                            HStack {
                                VStack(alignment: .leading, spacing: 4) {
                                    ...
                                    Text("Weight (kg): " + String(format: "%.1f", item.value2))
            ...
            .navigationTitle("Packing Weightly")
            .toolbar {
...
struct AddItemView: View {
    ...
    var body: some View {
        NavigationStack {
            Form {
                Section {
                    TextField("Bag Name", text: $titleText)
                        .focused($focusedField, equals: .title)
                        .accessibilityIdentifier("addTitleField")
                    TextField("Weight (kg)", text: $value2Text)
            ...
            .toolbar {
                ...
                ToolbarItem(placement: .confirmationAction) {
                    Button("Save") {
                        let item = Bag(title: titleText.isEmpty ? "Bag" : titleText, value2: Double(value2Text) ?? 0, notes: notesText)
                        if store.add(item) {
                            dismiss()
                        }
                    }
